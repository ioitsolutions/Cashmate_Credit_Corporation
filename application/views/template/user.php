...
<html>
	...
	<hr>
        ...
        <div id="menu">
            <ul id="nav">
                <?php $ctr=0; foreach ($links as $link => $url): ?>
                <li>
                    <?php
                        
                        $menus=Session::instance()->get('emp_menu');
                        foreach($menus as $menu)
                        {
                            if($menu==$link)
                            {
                                echo html::anchor($url, $link,array('id' => str_replace(' ','_',  strtolower($link))));
                                $ctr=$ctr+1;
                                if($ctr==6)
                                    echo'<br/>';
                            }
		...
		<?php foreach ($sublinks as $link => $url): ?>
                <li>
                    <?php
                        $submenus=Session::instance()->get('emp_submenu');
